...
import '../../models/variant.dart' show Variant;
import 'package:admin/models/category.dart' as admin_category;  // Alias your model
import 'package:flutter/foundation.dart' as flutter_foundation; // Alias the foundation
  ...
  Future<List<Coupon>> getAllCoupons({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredCoupons = List.from(_allCoupons); // Initialize filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
